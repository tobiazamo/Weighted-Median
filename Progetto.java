...
public class Progetto {
    public static void main(String[] args) {
        String s = (readLine());
        String[] arr = initializeArray(s);
        BigDecimal[] fArr = new BigDecimal[arr.length];
        for (int i = 0; i < arr.length; i++) {
            fArr[i] = BigDecimal.valueOf((Double.valueOf(arr[i])));
        }
        System.out.println(WeightedMedian(fArr));
    }

    /**
     * Inizializza l'array rimuovendo dall'input il punto alla fine
     * e dividendo ogni elemento attorno a (,)
     *
     * @param s a string
     * @return result an array containing the elements given in input
     */

    private static String[] initializeArray(String s) {
        String[] result;
